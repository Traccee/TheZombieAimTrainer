...
def draw_raygun(windowObj, mouse_pos):
    ...
    # Fsmall offset
    offset_x = (mouse_pos[0] - width / 2) // 10  # Adjust sensitivity as needed
    x_position += offset_x

    windowObj.blit(Raygun_Image, (x_position, y_position))
...
def main(): 
    ...
    while run: 
        ...
        for target in targets:
            ...
            if NumClicks and target.collide(*mouse_pos):  
               targets.remove(target)  
               target_hit += 1  

        if NumMisses >= NumLives: 
